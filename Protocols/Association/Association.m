...
LoadSerialMessages('Door1', {buzzer1, buzzer2, doorOpen, doorClose, ...
...
sma = AddState(sma, 'Name', 'CenterDelay', ...
    'Timer', S.GUI.PreOdorDelay,...
    'StateChangeConditions', {'Tup', OdorState,'Port2Out','WaitForCenter'},...
    'OutputActions', {});
sma = AddState(sma, 'Name', 'OdorCSPlus', ...
    'Timer', S.GUI.OdorTime,...
    'StateChangeConditions', {'Port2Out', 'OdorOffEarly', 'Tup', 'OdorOff'},...
    'OutputActions',[{'BNC2',1,'Door1',DIOmsg1},PresentOdor(0)]);
sma = AddState(sma, 'Name', 'OdorCSMinus', ...
    'Timer', S.GUI.OdorTime,...
    'StateChangeConditions', {'Port2Out', 'OdorOffEarly', 'Tup', 'OdorOff'},...
    'OutputActions',[{'BNC2',1,'Door1',DIOmsg1},PresentOdor(0)]);
sma = AddState(sma, 'Name', 'OdorOffEarly',...
    'Timer', 0,...
    'StateChangeConditions', {'Tup','WaitForCenter'},...
    'OutputActions', [{'Door1',DIOmsg2},PresentOdor(0),...
    PreloadOdor(Odor,0)]);
