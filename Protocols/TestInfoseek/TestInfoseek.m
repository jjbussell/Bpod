...
LoadSerialMessages('DIO1', {[254 1],[253 1],[5 1], [5 0]});
...
sma = AddState(sma, 'Name', 'StartTrial', ...
    ...
    'OutputActions', {'DIO1',1}); % {'Buzzer',1,'LED',1}buzzer on, light on (configure teensy, consider lighting center port)
...
sma = AddState(sma, 'Name', 'GoCue', ...
    ...
    'OutputActions', {'GlobalTimerTrig', 1,'DIO1',2}); % DOES TIMER START AT BEGINNING OR END? TIMER STARTS AT BEGINNING
...
sma = AddState(sma, 'Name', 'OdorLeft', ...
    ...
    'OutputActions', [{ 'PWM1',255}, turnOnSideOdor(LeftSideOdor,'left')]);
...
sma = AddState(sma, 'Name', 'OdorRight', ...
    ...
    'OutputActions', [{'PWM3',255}, turnOnSideOdor(RightSideOdor,'right')]);
